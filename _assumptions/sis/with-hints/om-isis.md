...
_Let matrix $\mat{A} \in \ZZ_q^{n \times m}$ and $T \subset \ZZ_q^n$ be chosen uniformly at random. Given the challenge matrix $\mat{A}$ and the set of target vectors $T$, an adversary can query a preimage oracle $O\_\text{pre}$ adaptively, which on input $\hat{\vec{t}} \in \ZZ_q^n$ outputs a preimage $\hat{\vec{s}} \sample \mat{A}\_s^{-1}(\hat{\vec{s}})$. Let $k \in \NN_0$ denote the number of times $O\_\text{pre}$ was queried. Then, an adversary is asked to output a set $\set{\vec{s}_i}\_{i \in [k+1]}$ of $k+1$ short preimages of target vectors in $T$ satisfying_
