...
This condition encompasses linear leakages over both the secret $$\vec{s}$$ and the error $$\vec{e}$$. In this setup, $$\chi_\textsf{xxx}$$ are distinct (elliptical) discrete Gaussian distributions.
...
If the leakage is only revealing information over the error $$\vec{e}$$, then the reduction can be done for any secret distribution, as long as $$\chi_\mathbf{e},\chi_\mathbf{e'}$$, and the noise $$\chi_\mathbf{y}$$ are distinct (elliptical) discrete Gaussian distributions. 
...
The value of the bound $$\beta$$ depends on the different standard deviations. 
